...
export const validateMove = (board: Board, move: Move, mapHeight: number, mapWidth: number): boolean => {
  // TODO
  // Move has to:
  ...
  // 1.
  if(coordY < 0 || coordY >= mapHeight
    || coordX < 0 || coordX >= mapWidth){
      return false;
    }

  // 2.
  const secondBoxNeighbour = move.type === 'horizontal' ?
    `${coordY-1},${coordX}`
    : `${coordY},${coordX-1}`

  if(board.boxes[secondBoxNeighbour] !== undefined 
