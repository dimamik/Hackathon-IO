...
export function computeEnclosedBoxes(
  ...
  mapWidth: number,
  mapHeight: number): Array<string> {  

  if(clickHorizontal){
    if(mapState.horizontalBars.get(clickedPosition) === undefined){
      throw Error("Clicked position has to be saved to map state before calling computeEnclosedBoxes");
    }
  }
  else{
  ...
  const boxIsInMap = (boxPosition: [number, number]): boolean => {
    return boxPosition[0] >= 0 && boxPosition[1] >= 0 
            && boxPosition[0] < mapHeight && boxPosition[1] < mapWidth
  }
      
  let enclosedBoxes = Array<string>();

  const searchEnclosedBoxes = (mapState: MapState, fromPosition: [number, number], mapHeight: number, mapWidth: number): Array<string> => {
